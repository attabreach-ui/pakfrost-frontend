...
import { Search, Filter, TrendingDown, TrendingUp, BarChart3, Printer, Edit2, X, Save, Eye } from 'lucide-react';
import type { StockMovement, Pallet, Customer, Product, Driver, Vehicle } from '@/types'; // H4 FIX: import proper types
...
export default function HistoryPage({ movements, pallets, customers, products, drivers, vehicles: _vehicles, onEditIGP, onEditOGP, onFetchIGPDetail, onFetchOGPDetail }: HistoryPageProps) {
  ...
  const PAGE_SIZE = 50;
  const [editLines, setEditLines] = useState<any[]>([]); // OGP line quantities for editing // Pagination: show 50 records per page
  ...
  const stats = useMemo(() => ({
    totalIn:  filteredMovements.filter(m => m.type === 'IN').reduce((s, m) => s + Number(m.cartons), 0),
    totalOut: filteredMovements.filter(m => m.type === 'OUT').reduce((s, m) => s + Number(m.cartons), 0),
  ...
  const ledgerData = useMemo(() => {
    ...
    const totalIn   = custMoves.filter(m => m.type === 'IN').reduce((s, m) => s + Number(m.cartons), 0);
    const totalOut  = custMoves.filter(m => m.type === 'OUT').reduce((s, m) => s + Number(m.cartons), 0);
                ...
                <table className="w-full text-sm min-w-[900px]">
                  <thead>
                    <tr style={{ background: 'var(--bg-input)' }}>
                      {['Date','Doc No','Type','Customer','Product','Qty','Weight','Location','Vehicle','Driver','Edit'].map(h => (
                  ...
                  <tbody>
                    {paginatedMovements.map(m => (
                      <tr key={m.id} className="hover:bg-white/5 transition-colors" style={{ borderTop: '1px solid rgba(43,184,232,0.05)' }}>
                        ...
                        <td className="px-3 py-2.5 font-bold" style={{ color: m.type==='IN' ? '#4ade80' : '#f97316' }}>
